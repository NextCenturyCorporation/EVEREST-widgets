...
(function() {
    var rawPatientReminderListTemplate     = "<div class='panel-heading'><h4>Reminders for {{ name }}<button class='btn btn-xs pull-right' data-toggle='modal' data-target='#remListModal'>?</button></h4></div><div class='panel-body'><ul class='nav nav-pills nav-stacked'></ul></div>"
      ...
      , rawReminderFormTemplate            = "<div class='panel-heading'><h4>Reminder Resolution<button class='btn btn-xs pull-right' data-toggle='modal' data-target='#remFormModal'>?</button></h4><h5>{{ performedBy }} - {{ title }}</h5></div><div class='panel-body'><div class='well well-sm'><table class='table table-condensed'><thead><th>Who</th><th>What</th><th>When</th></thead><tbody>{{#each components}}<tr><td>{{ who }}</td><td>{{ what }}</td><td>{{ when }}</td></tr>{{/each}}</tbody></table></div><form role='form'><div class='well well-sm'><h4>Resolution:</h4><div id='resolution-controls'></div></div><div class='well well-sm'><h4>Exceptions:</h4>{{#each exceptions}}<div class='radio'><label><input type='radio' name='optionsRadios' id='optionsRadios{{ @index }}' value='option{{ @index }}'>{{ . }}</label></div>{{/each}}</div></form><button id='submitReminderBtn' class='btn btn-default'>Submit</button></div>"
