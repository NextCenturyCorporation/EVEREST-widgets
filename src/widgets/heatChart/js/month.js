...
month_heatChart_widget.execute = function() {
	...
	for (var i = 1; i < DAYS_PER_MONTH + 1; i++) {
	...
	owfdojo.addOnLoad(function(){
		OWF.ready(function(){
			OWF.Eventing.subscribe("com.nextcentury.everest.data_table_announcing.raw_data", function(sender, msg){
				...
				month_heatChart_widget.createChart(date_list, raw_data, month_chunks, month_labels, day_labels);
			});
		});
	});
};

month_heatChart_widget.createChart = function(date_list, raw_data, month_chunks, month_labels, day_labels) {
	...
	month_heatChart_widget.drawChart(month_labels, month_chunks, day_labels);

};

month_heatChart_widget.drawChart = function(month_labels, month_chunks, day_labels) {
